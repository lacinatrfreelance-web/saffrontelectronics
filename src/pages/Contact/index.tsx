...
export const ContactPage: React.FC = () => {
  ...
  const CONTACT_CARDS = [
    {
      icon: Phone,
      label: t('contact.cardPhoneLabel'),
      value: COMPANY.phone,
      sub: t('contact.cardPhoneSub'),
    ...
    {
      icon: MapPin,
      label: t('contact.cardShowroomLabel'),
      value: t('contact.cardShowroomValue'),
      sub: COMPANY.address,
      href: null,
    ...
    {
      icon: Clock,
      label: t('contact.cardHoursLabel'),
      value: t('contact.cardHoursValue'),
      sub: t('contact.cardHoursSub'),
      href: null,
        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-5 gap-10">
            ...
            <div className="lg:col-span-3">
              ...
              >
                <p className="text-xs font-black text-orange-500 uppercase tracking-[0.2em] mb-3">{t('contact.writeSectionBadge')}</p>
                <h2 className="text-3xl font-black text-gray-900 mb-2">{t('contact.writeSectionTitle')}</h2>
                <p className="text-gray-400 text-sm mb-8">{t('contact.writeSectionSubtitle')}</p>
              </motion.div>
              <ContactForm />
            </div>
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-xs font-bold uppercase tracking-[0.15em] opacity-70 mb-1">{t('contact.followUs')}</p>
              ...
              >
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <span className="text-sm font-bold text-emerald-500">{t('contact.openNow')}</span>
                </div>
                <div className="space-y-3">
                  {[
                    { dayKey: 'contact.weekdays', hoursKey: 'contact.cardHoursValue' },
                    { dayKey: 'contact.sunday',   hoursKey: 'contact.cardHoursSub'   },
                  ].map(({ dayKey, hoursKey }) => (
                    <div key={dayKey} className="flex justify-between items-center text-sm">
                      <span className="text-gray-500">{t(dayKey)}</span>
                      <span className="font-bold text-gray-900">{t(hoursKey)}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
