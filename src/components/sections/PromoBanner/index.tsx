...
export const PromoBanner: React.FC<PromoBannerProps> = ({ promotion }) => {
  ...
  return (
    <section className="relative overflow-hidden py-16">
      <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #F59E0B 0%, #F97316 50%, #EF4444 100%)' }} />

      <motion.div animate={{ scale: [1, 1.2, 1], rotate: [0, 10, 0] }} transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
        className="absolute -top-36 -right-36 w-96 h-96 rounded-full bg-white/10 pointer-events-none"
      />
      <motion.div animate={{ scale: [1.15, 1, 1.15], rotate: [6, 0, 6] }} transition={{ duration: 16, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
        className="absolute -bottom-36 -left-36 w-[420px] h-[420px] rounded-full bg-white/8 pointer-events-none"
      />

      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="absolute h-px bg-white/10" style={{ width: '200%', top: `${15 + i * 16}%`, left: '-50%', transform: 'rotate(-14deg)' }} />
        ))}
      </div>
      ...
      <div className="container-custom relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10">
          ...
          <div className="text-center lg:text-left max-w-xl">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.65 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.28, type: 'spring', stiffness: 260, damping: 18 }}
            ...
            >
              ...
              -{promotion.discount_percentage}% {t('promo.discount')}
            </motion.div>

            <div className="block">
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                <Link to="/promotions" className="inline-flex items-center gap-2.5 bg-white/15 hover:bg-white/28 border border-white/30 text-white font-black text-sm px-7 py-3.5 rounded-2xl transition-all duration-200">
                  {t('promo.viewOffers')}
                  <ArrowRight size={15} />
                </Link>
              </motion.div>
          ...
          {/* ── RIGHT — countdown ── */}
          <motion.div initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.6 }}
          ...
          >
            ...
            <div className="flex items-end gap-2">
              <Digit value={countdown.days}    label={t('promo.days')}    />
              <span className="text-white/35 font-black text-3xl pb-9">:</span>
              <Digit value={countdown.hours}   label={t('promo.hours')}   />
              <span className="text-white/35 font-black text-3xl pb-9">:</span>
              <Digit value={countdown.minutes} label={t('promo.minutes')} />
              <span className="text-white/35 font-black text-3xl pb-9">:</span>
              <Digit value={countdown.seconds} label={t('promo.seconds')} />
            </div>
          </motion.div>
        </div>
      </div>
